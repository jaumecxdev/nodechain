...
async function main() {
    ...
    var transaction = LCLTCrypto.signTransaction(userAccount.private, {
        ...
        payload: JSON.stringify({ 
            coin_address: coin.address,     // ALSO: orgAccount.coin_address
            org_address: orgAccount.address,
            count: 10000
        }),
    });

    await grpcCoin.mine(transaction, async (err, response) => {
        if (err) {
            ...
            if (response.code == grpc.status.OK) {
                ...
                console.log("Org: " + JSON.stringify(payload.org));
            }
