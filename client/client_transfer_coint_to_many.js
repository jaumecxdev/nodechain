...
async function main() {
    ...
    var allUsersAccount = await World.find({ datatype: 'account', ['info.type']: 'users' }).exec();
    
    var tos = [];
    for (const userAccount of allUsersAccount) {
        tos.push(userAccount.address);
    }
    ...
    // Org -> All Users    
    grpcCoin.transferTos(transaction, async (err, response) => {
        if (err) {
            ...
            if (response && response.code == grpc.status.OK) {
                ...
                //console.log("To: " + JSON.stringify(payload.tos.balance));
            }
        }
    });
}
